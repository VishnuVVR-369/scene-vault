...
describe("library state", () => {
  it("creates nested folders and resolves folder paths", () => {
    ...
    state = createFolder(state, { name: "Product", parentFolderId: null }, { id: "f1", now: 1 });
    state = createFolder(state, { name: "Auth", parentFolderId: "f1" }, { id: "f2", now: 2 });
  ...
  it("prevents moving a folder into one of its descendants", () => {
    let state = createInitialLibraryState("user_1");
    state = createFolder(state, { name: "A", parentFolderId: null }, { id: "a", now: 1 });
    state = createFolder(state, { name: "B", parentFolderId: "a" }, { id: "b", now: 2 });
  ...
  it("permanently deletes nested folders and scenes", () => {
    ...
    state = createFolder(state, { name: "A", parentFolderId: null }, { id: "a", now: 1 });
    state = createFolder(state, { name: "B", parentFolderId: "a" }, { id: "b", now: 2 });
    state = createScene(state, { title: "Diagram", folderId: "b" }, { id: "s1", now: 3 });
  ...
  it("searches only scene titles and folder paths", () => {
    let state = createInitialLibraryState("user_1");
    state = createFolder(state, { name: "Hiring", parentFolderId: null }, { id: "f1", now: 1 });
    state = createScene(state, { title: "Onboarding", folderId: "f1" }, { id: "s1", now: 2 });
    state = createScene(state, { title: "Roadmap", folderId: null }, { id: "s2", now: 3 });

    expect(filterScenes(state, "hiring", null).map((scene) => scene.id)).toEqual(["s1"]);
    expect(filterScenes(state, "road", null).map((scene) => scene.id)).toEqual(["s2"]);
  });

  it("duplicates metadata without carrying saved object pointers", () => {
    let state = createInitialLibraryState("user_1");
    state = createScene(state, { title: "Flow", folderId: null }, { id: "s1", now: 1 });
    state.scenes[0] = {
      ...state.scenes[0],
  ...
  it("stores scene bundles without Excalidraw collaborators app state", () => {
    ...
    expect(state.bundles.s1.appState).toEqual({ viewBackgroundColor: "#ffffff" });
  });
});
